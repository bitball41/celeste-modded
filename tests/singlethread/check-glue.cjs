...
try {
  for (const quote of ["'", '"']) for (const indent of ['', '  ', '    ']) {
    ...
    assert(first.includes('Module["_test"] = (...args)'));
    assert(first.indexOf('Module["_test"] = (...args)') < first.indexOf("if (Module['preInit'])"));
